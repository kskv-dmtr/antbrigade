/* Сводный указатель для поиска в шапке.
   ...
   у исполнителя, лейбла и жанра — обложка одного из их релизов, выбранная
   наугад при сборке, у исполнителя без релизов — кадр клипа. Нет скачанной картинки — пусто.
...
/* Обложка для находки берётся наугад из всех релизов записи — с 20 сентября
   2026 (просьба владельца); до того бралась обложка самого свежего. У кого
   релиз один, выбирать не из чего, и эта же обложка стоит всегда.

   Жребий бросается при сборке, а не в браузере: указатель — готовый файл.
   Сайт пересобирается ежечасно, так что обложки время от времени меняются
   сами — как «Random Picks» на витрине. */
const наугад = (ids = []) => {
  const есть = ids.map((id) => albumById.get(id)).filter((al) => al && обложка(al));
  return есть.length ? есть[Math.floor(Math.random() * есть.length)] : null;
};
const миниатюра = (albumIds, videoIds = []) => {
  const al = наугад(albumIds);
  if (al) return обложка(al);
